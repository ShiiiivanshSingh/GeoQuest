...
const Game = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      {/* Small Screen Warning - Only visible on screens below 768px */}
      <div className="md:hidden fixed inset-0 bg-black/90 backdrop-blur-md z-[200] flex flex-col items-center justify-center p-6 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-yellow-500 mb-4 pulse-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <h2 className="text-2xl font-bold text-white mb-2">Screen Too Small</h2>
        <p className="text-white/80 mb-4">
          GeoQuest works best on larger screens. For the best experience:
        </p>
        <div className="bg-white/10 rounded-lg p-4 mb-6 max-w-md text-left">
          <ol className="list-decimal list-inside space-y-2 text-white/80">
            <li>Open your browser settings (usually in the top-right menu)</li>
            <li>Find the "<strong className="text-white">Desktop site</strong>" or "<strong className="text-white">Desktop view</strong>" option</li>
            <li>Enable this setting to view the full game experience</li>
          </ol>
        </div>
        
        <div className="flex gap-4">
          <button 
            className="bg-white text-black font-bold py-2 px-6 rounded-lg shadow-lg hover:bg-white/90 transition-all"
            onClick={() => window.location.href = "/"}
          >
            Return to Home
          </button>
          <button 
            className="bg-transparent text-white font-bold py-2 px-6 rounded-lg shadow-lg border border-white/30 hover:bg-white/10 transition-all"
            onClick={() => {
              const warningElement = document.querySelector('.md\\:hidden');
              if (warningElement) {
